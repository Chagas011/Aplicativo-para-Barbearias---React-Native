...
export function StepWorkingHours() {
  const { control, setValue } = useFormContext<CreateBarberFormData>();
  ...
  function openPicker(index: number, type: "start" | "end") {
    setPicker({
      index,
      type,
      visible: true,
  ...
  function handleConfirm() {
    ...
    setValue(`workingHours.${picker.index}.${picker.type}`, formatted, {
      shouldValidate: true,
    });
        ...
        <View key={field.id} style={{ marginBottom: 10 }}>
          ...
          <Controller
            ...
            render={({ field: { value } }) => (
              ...
              >
                <ThemedText>
                  {value ? `entrada: ${value}` : "Selecionar entrada"}
                </ThemedText>
              </Pressable>
            )}
          />
          ...
          <Controller
            ...
            render={({ field: { value } }) => (
              ...
              >
                <ThemedText>
                  {value ? `Saida: ${value}` : "Selecionar saida"}
                </ThemedText>
              </Pressable>
            )}
          />
